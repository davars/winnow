...
**Deviations:**
- Phase 2's `EnsureSchema` had a latent multi-provider bug: it dropped any column not declared by the current provider (excluding base columns), so running `sha256` and then `mime` wiped sha256's columns. Phase 9 surfaced this because the enricher framework reuses `EnsureSchema` for enricher-owned tables while sha256 and mime still shared the `files` table. Resolved architecturally by introducing a single-owner `filesProvider` in `db/db.go` that declares every extension column on `files` (sha256, hashed_at, mime_type, mime_checked_at) plus the `files_sha256` index. `db.Open` calls `EnsureSchema(db, filesProvider{})` after the core `CREATE TABLE`s, and the `sha256`/`mime` packages no longer declare `SchemaProvider`s — they just write into columns the db package owns. `getBaseColumns("files")` unions the hardcoded core columns with `filesProvider{}.Columns()` so any other provider accidentally targeting `files` (e.g. a test) cannot drop them.
- `FetchBatch` does not use the stored `file_id` for path resolution (the plan calls it "historical/audit" — a file_id may have moved or gone missing since identify). Instead it joins on `files.sha256 = exif.hash AND files.missing = 0` and picks the lowest `files.id` via a correlated subquery. This means enrichers automatically retry with a different copy of the content if the originally-identified file is gone.
- The WriteBatch is generated automatically from the enricher's declared columns: it always sets `processed_at = now` (success or error) and writes the declared columns from `WorkResult.Values`. Unknown keys in `Values` cause the transaction to be rolled back with an error. On per-item errors, the row is marked processed (with column values NULL) and the error logged to `process_errors` — mirrors sha256/mime retry semantics.
- `decodeExifString` tolerates numeric JSON forms: exiftool occasionally emits numbers for `Make`/`Model` on some cameras. Falls back to the raw JSON bytes in that case.
